...
static int mzip_load_central(zip_t *za) {
	...
	for (i = 0; i < n_entries; ++i) {
		...
		e->name = r_str_ndup (h + 46, filename_len);
...
zip_file_t *zip_fopen_index(zip_t *za, zip_uint64_t index, zip_flags_t flags) {
	...
	zip_file_t *zf = R_NEW (zip_file_t);
	zf->data = buf;
	zf->size = sz;
	return zf;
}
...
zip_source_t *zip_source_buffer(zip_t *za, const void *data, zip_uint64_t len, int freep) {
	(void)za;
	zip_source_t *src = R_NEW0 (zip_source_t);
	if (!src) {
		return NULL;
	}
	src->buf = data;
...
/* Override zip_close to finalize archive when in write mode */
int zip_close(zip_t *za) {
	if (!za) {
		return -1;
	}
